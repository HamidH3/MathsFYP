...
# background_path = os.path.join(os.path.dirname(__file__), 'background_image.webp')
...
#     # Return frame data
#     with open("frame.png", "rb") as img_file:
...
class Ball:
    ...
    def update(self):
        ...
        # Prevent ball from falling below ground
        if self.y > HEIGHT - BALL_RADIUS - 10:
            self.y = HEIGHT - BALL_RADIUS - 10
            self.on_ground = True
            self.velocity = 0
        else:
    ...
    def draw(self, screen):
        pygame.draw.circle(screen, RED, (self.x, int(self.y)), BALL_RADIUS)

# Game loop function for a single frame
def get_game_frame():
    clock = pygame.time.Clock()
    ball = Ball()
